...
class ReportsController < ApplicationController
  def weekly_stats
    ...
    @lords_days_data = LordsDay.where(date: @group_start_date..@end_date)
                                ...
                                .map { |week, instances| ["Week #{week.split('-').first.to_i - 1}, #{week.split('-').last}", instances.sum(&:total), instances.sum(&:adults), instances.sum(&:teenagers), instances.sum(&:children)] }
                                .sort_by { |week, _total, _adults, _teenagers, _children|  Date.strptime(week, "Week %U, %Y") }
    ...
    @prayer_meetings_data = PrayerMeeting.where(date: @group_start_date..@end_date)
                                          ...
                                          .map { |week, instances| ["Week #{week.split('-').first.to_i}, #{week.split('-').last}", instances.sum(&:total), instances.sum(&:adults), instances.sum(&:teenagers), instances.sum(&:children)] }
                                          .sort_by { |week, _total, _adults, _teenagers, _children|  Date.strptime(week, "Week %U, %Y") }
   ...
   @current_week_small_groups_data = SmallGroup.where(date: @start_date..@end_date)
                                                ...
                                                .transform_values do |small_groups|
                                                  ...
                                                  { adults: adults, teenagers: teenagers, children: children, total: total }
                                                end
